...
// "Print" opens the browser dialog, where the customer can also "Save as PDF".
export default function ReceiptActions({ downloadHref }: { downloadHref: string }) {
  ...
  const solid: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 700, padding: '9px 16px', borderRadius: 10, border: 'none', color: '#fff', background: '#C9A84C', cursor: 'pointer' }
  return (
    <div className="no-print" style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginBottom: 16, flexWrap: 'wrap' }}>
      ...
      <button type="button" onClick={() => window.print()} style={solid}><Printer size={14} /> Print / Save as PDF</button>
    </div>
  )
}
