  ...
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
      ...
      <div className="relative rounded-3xl border overflow-hidden h-full transition-all duration-300 hover:-translate-y-1.5 hover:shadow-product-hover"
        ...
        <Link href={`/shop/${p.slug}`} className="block relative overflow-hidden group" style={{ aspectRatio: featured ? '4/3' : '3/2', background: '#000' }}>
          <Image src={p.thumbnail || FALLBACK_IMG} alt={p.title} fill sizes="(max-width:1024px) 100vw, 33vw" className="object-contain transition-transform duration-500 group-hover:scale-105" priority={featured} />
          {/* hot this week */}
