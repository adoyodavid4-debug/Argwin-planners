              ...
              {recentlyViewed.map((p) => (
                <Link key={p.id} href={`/shop/${p.slug}`} className="flex-shrink-0 w-40 group">
                  <div className="relative rounded-xl overflow-hidden mb-2" style={{ aspectRatio: '3/4', background: '#000', border: '1px solid var(--border)' }}>
                    <Image src={p.thumbnail || FALLBACK_IMG} alt={p.title} fill sizes="160px" className="object-contain transition-transform duration-300 group-hover:scale-105" />
                  </div>
...
function ProductRow({ product, index, inCompare, onCompare, onQuickView }: CardProps) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: Math.min(index * 0.03, 0.2) }}
      ...
      <Link href={`/shop/${product.slug}`} className="relative flex-shrink-0 rounded-xl overflow-hidden" style={{ width: 110, aspectRatio: '3/4', background: '#000' }}>
        <Image src={product.thumbnail || FALLBACK_IMG} alt={product.title} fill sizes="110px" className="object-contain" />
      </Link>
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: `120px repeat(${products.length}, minmax(0,1fr))` }}>
            ...
            {products.map((p) => (
              <div key={p.id} className="text-center">
                <Link href={`/shop/${p.slug}`} className="block relative rounded-lg overflow-hidden mb-2 mx-auto" style={{ aspectRatio: '3/4', maxWidth: 120, background: '#000' }}>
                  <Image src={p.thumbnail || FALLBACK_IMG} alt={p.title} fill sizes="120px" className="object-contain" />
                </Link>
