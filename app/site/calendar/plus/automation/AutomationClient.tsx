...
import { Palette, Repeat, Bell, Tag, Plus, X, Check } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { saveSettings } from '@/lib/calendar/settings'
import PlusShell, { StatCard, SectionCard, Switch } from '../PlusShell'
import { type PlusWorkspace, type AutomationRule, type AutomationKind, autoKey, customAutoRulesKey } from '@/lib/calendar/plus'
...
const blankForm = { label: '', detail: '', kind: 'colour' as AutomationKind }
...
export default function AutomationClient({ ws }: { ws: PlusWorkspace }) {
  ...
  const toggle = (id: string) => {
    ...
    if (ws.live) {
      const next = { ...features, [autoKey(id)]: on }
      setFeatures(next)
  ...
  const saveRule = () => {
    ...
    setRules((rs) => [...rs, rule])
    if (ws.live) {
      const customs: AutomationRule[] = Array.isArray(features[customAutoRulesKey]) ? features[customAutoRulesKey] : []
      const next: Record<string, any> = { ...features, [customAutoRulesKey]: [...customs, rule] }
      setFeatures(next)
      saveSettings(supabase, { features: next })
    }
    setForm(null)
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6" style={{ background: 'rgba(0,0,0,0.45)' }} onClick={() => setForm(null)}>
          <div className="w-full sm:max-w-md max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl border p-6" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <Field label="Name *"><input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} className={inp} style={inpStyle} placeholder="Colour client calls" /></Field>
              <Field label="Type">
                <div className="flex gap-1.5 flex-wrap">
                  {(Object.keys(KIND) as AutomationKind[]).map((k) => {
              ...
              <Field label="What it does"><textarea value={form.detail} onChange={(e) => setForm({ ...form, detail: e.target.value })} rows={2} className={inp + ' resize-none'} style={inpStyle} placeholder="e.g. “client” events turn Clay, with a 30-min reminder." /></Field>
