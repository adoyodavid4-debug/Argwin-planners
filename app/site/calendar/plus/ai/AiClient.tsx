...
import { BellRing, MessageSquare, Clock, Brain, ShieldCheck, Check, X, Sparkles } from 'lucide-react'
...
export default function AiClient({ ws }: { ws: PlusWorkspace }) {
  ...
  const accept = (id: string) => { setItems((xs) => xs.filter((x) => x.id !== id)); setDone((d) => d + 1); toast.success('Applied to your calendar') }
                ...
                {items.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="flex flex-wrap items-start gap-3 rounded-xl border p-3.5" style={{ borderColor: 'var(--border)' }}>
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => accept(s.id)} className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-semibold text-white" style={{ background: 'var(--gold)' }}><Check size={13} /> Apply</button>
