...
import { Clock, Globe, Users, ChevronLeft, ChevronRight, Check, CalendarCheck, Download, ArrowLeft, Loader2 } from 'lucide-react'

export interface PublicTeamPage {
  teamId: string
  slug: string
  name: string
  type: 'round-robin' | 'collective' | 'group'
  duration_min: number
  description: string | null
  teamName: string
  timezone: string
  hosts: { id: string; name: string; hue: number }[]
}
...
const TYPE_BLURB: Record<PublicTeamPage['type'], string> = {
  'round-robin': 'You’ll be matched with whichever host is free.',
  collective: 'Books a time when the whole team is available.',
  group: 'A shared session with the team.',
}
...
function downloadIcs(r: Result) {
  ...
  const ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Arwign//Teams//EN', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', 'BEGIN:VEVENT',
    `UID:${r.id}@arwignplanners.com`, `DTSTAMP:${stamp(new Date().toISOString())}`, `DTSTART:${stamp(r.startISO)}`, `DTEND:${stamp(r.endISO)}`,
    `SUMMARY:${esc(r.title)}`, `DESCRIPTION:${esc('Booked with Arwign Teams')}`, 'END:VEVENT', 'END:VCALENDAR']
    .filter(Boolean).join('\r\n')
...
export default function TeamBookingClient({ page }: { page: PublicTeamPage }) {
  ...
  const loadSlots = useCallback(async (d: string) => {
    ...
    try {
      const res = await fetch(`/api/calendar/team-bookings?team=${encodeURIComponent(page.teamId)}&slug=${encodeURIComponent(page.slug)}&date=${d}`)
      const json = await res.json()
      setSlots(res.ok ? json.slots ?? [] : [])
    } catch { setSlots([]) } finally { setLoading(false) }
  }, [page.teamId, page.slug])
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch('/api/calendar/team-bookings', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ team: page.teamId, slug: page.slug, startISO: slot.startISO, name: form.name, email: form.email, notes: form.notes, guestTz }),
      })
      const json = await res.json()
      ...
      <div className="min-h-[80vh] flex items-center justify-center px-6 py-16" style={{ background: 'var(--bg-primary)' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg rounded-3xl border p-8 text-center" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          ...
          <div className="rounded-2xl border p-5 text-left mb-6" style={{ borderColor: 'var(--border)', background: 'var(--bg-secondary)' }}>
            ...
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Times shown in {result.timezone}{guestTz !== result.timezone ? ` · ${timeInTz(result.startISO, guestTz)} your time` : ''}</p>
          </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid lg:grid-cols-[1fr,1.4fr] gap-6 lg:gap-8">
          ...
          <div className="rounded-3xl border p-6 h-fit lg:sticky lg:top-24" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
            <p className="text-xs uppercase tracking-widest font-semibold mb-3" style={{ color: accent, letterSpacing: '0.12em' }}>{page.teamName}</p>
            <h1 className="font-display text-2xl mb-3" style={{ color: 'var(--text-primary)' }}>{page.name}</h1>
            <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--text-secondary)' }}>{page.description || TYPE_BLURB[page.type]}</p>
            <ul className="space-y-2.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
              ...
              <li className="flex items-start gap-2.5">
                <Users size={15} style={{ color: accent }} className="mt-0.5" />
                  ...
                  {page.hosts.map((h) => (
                    <span key={h.id} className="inline-flex items-center gap-1.5">
                      <span className="flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-semibold"
                        style={{ background: `hsl(${h.hue} 45% 92%)`, color: `hsl(${h.hue} 45% 34%)` }}>{initials(h.name)}</span>
                ...
                <motion.div key="form" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
                  ...
                  <div className="space-y-3.5">
                    ...
                    <button onClick={submit} disabled={submitting} className="btn-primary w-full justify-center">
                      {submitting ? <><Loader2 size={15} className="animate-spin" /> Booking…</> : <>Confirm booking <Check size={15} /></>}
                    </button>
                    <p className="text-[11px] text-center" style={{ color: 'var(--text-muted)' }}>
                      Times shown in {page.timezone}. You’ll get a calendar invite to add.
                    </p>
                  </div>
                </motion.div>
              )}
