...
export async function POST(req: NextRequest) {
  ...
  const { name, email, idea, locale } = parsed.data
  const supabase = createServiceRoleClient()

  const { error } = await supabase
    .from('notebook_requests')
    .insert({ name, email, idea, locale })

  if (error) {
    console.error('[notebook-requests]', error)
  ...
  if (adminEmail) {
    try {
      ...
      // Don't fail the request — the idea is saved either way
    }
  }
