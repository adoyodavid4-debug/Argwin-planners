...
export async function POST(req: NextRequest) {
  ...
  if (orderErr || !order) {
    console.error('[paystack/initialize] order insert failed:', orderErr)
  ...
  try {
    // reference = our order id, so the webhook/return can reconcile by primary key.
    const tx = await initializePaystackTransaction({
      email,
      amount:      total,
      reference:   order.id,
      callbackUrl: `${appUrl}/checkout/paystack/return?order=${order.id}`,
    })

    return NextResponse.json({ redirectUrl: tx.authorization_url, orderId: order.id })
  } catch (err) {
